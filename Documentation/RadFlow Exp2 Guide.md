...
The prototype is built as an **edge-native multimodal RAG pipeline** optimized for unreliable connectivity and consumer-grade GPUs. In practice, the system behaves like a compact “middleware + pre-processor + VLM” stack:

1. **Ingestion (UI):** the nurse uploads a CXR (DICOM/JPEG) and enters the clinical prompt (e.g., “Evaluate for active TB”).
2. **Safety + routing (middleware):** the text is embedded, the vector DB retrieves relevant guidelines, and the retrieved context is appended to the system prompt.
3. **Foveal pre-processing (vision efficiency):** the image is converted into **two views**: (a) a low-res global thumbnail for context, and (b) a high-res crop around the densest suspicious region. This reduces visual token bloat while preserving diagnostically important detail.
4. **Inference (VLM):** both images + text context are passed to the multimodal model; the system emits a structured “draft preliminary report” plus a confidence/uncertainty signal.
5. **Follow-ups (interactive MedVQA):** subsequent nurse questions reuse cached visual representations when possible for faster turn-by-turn responses.

#### **4.2.1 Runtime Engine Selection (Hardware-Aware)**

To increase robustness across hackathon laptops and clinic workstations, RadFlow-Edge can select an inference backend at runtime based on detected hardware:

* **NVIDIA GPU available:** Transformers + 4-bit quantization (bitsandbytes).
* **Apple Silicon / unified memory environments (future portability):** MLX-based VLM backends.
* **No accelerator:** fall back to a smaller model (e.g., Moondream2 / distilled variants) to keep latency acceptable.

This “hardware detective” approach reduces the risk of demo failure when the runtime environment differs from the training environment.

#### **4.2.2 Quantization Strategy (4-bit / QLoRA-style)**

Running an 8B-class model on consumer hardware requires aggressive optimization. We employ **4-bit quantization** (e.g., NF4 via bitsandbytes).17

* **Mechanism:** compress model weights from FP16/FP32 down to 4-bit.
* **Impact:** reduces VRAM requirements enough to run on mid-range GPUs typical of field-deployable laptops.
* **Tradeoff:** small accuracy degradation is possible; safety guardrails (RAG + uncertainty) are treated as first-class mitigations.

#### **4.2.3 Retrieval-Augmented Generation (RAG) Layer**

To reduce hallucinations and align outputs to local practice, we implement **RAG** over a local vector store (e.g., ChromaDB/FAISS).18

* **Knowledge base:** WHO TB guidance, pneumonia standards, and vetted reporting templates.
* **Prompt grounding:** retrieved clinical guidance is injected into the system prompt so the model answers using explicit, auditable references rather than implicit memory alone.

#### **4.2.4 Uncertainty Quantification (UQ)**

Safety is paramount. The system computes an uncertainty signal (e.g., via multi-sample generation and semantic divergence).19

* **If high uncertainty:** the UI flags the case for specialist review instead of encouraging decisive action.
* **If low uncertainty:** the UI permits a nurse to proceed with standing orders, while keeping all outputs labeled as “draft preliminary findings.”

#### **4.2.5 Foveal Cropping + Visual Token Efficiency (Vision Hack)**

High-resolution CXRs can be token-expensive for VLMs. To keep inference stable on edge hardware, the pipeline uses a **foveal strategy**:

* **Global context view:** aggressively downsample the full image for “where is what” context.
* **High-res anomaly crop:** use a fast heuristic (e.g., contrast/edge density) to crop a native-resolution patch around the most suspicious region.

This preserves fine detail (e.g., cavitation margins) while reducing the overall visual token load.

#### **4.2.6 Visual/KV Caching for Fast Follow-ups**

For multi-turn MedVQA, the system can cache the visual embeddings / KV cache for the initial image tokens so follow-up questions avoid re-encoding the same pixels. This improves responsiveness and makes interactive “second look” workflows feel practical.
...
### **4.4 Future Improvement: D-RoVA (Dynamic Routing for Visual Attention in Medical VLMs)**

The foveal crop strategy reduces token bloat **outside** the model. A higher-impact research direction is to reduce token bloat **inside** the model by replacing a static vision-to-LLM connector with a lightweight, text-guided token router:

1. **Text-guided routing:** before expensive autoregressive decoding, a router scores visual tokens for relevance to the prompt (e.g., “apical cavitation”).
2. **Dynamic dropping:** low-relevance tokens (e.g., abdomen/background) are dropped; high-relevance tokens (e.g., lung apices) are retained.
3. **Differentiable pruning:** routing is learnable, enabling training-time optimization rather than hard-coded cropping heuristics.

This direction (D-RoVA) aims to preserve clinical fidelity while improving latency and stability on edge devices.
